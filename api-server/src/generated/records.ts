...
export const SignalReview = z.object({ "input_record_id": UuidV4.optional(), "note": z.string().min(1) }).strict();
...
export const ReflectingOutcome = z.object({ "actual_result": z.string().min(1), "expected_result": z.string().min(1), "pnl_delta": z.number().optional() }).strict();
export type ReflectingOutcome = z.infer<typeof ReflectingOutcome>;

export const ReflectingRecord = z.intersection(BaseRecord, z.object({ "behavior": z.literal("Reflecting"), "reflected_on_acting_record_id": UuidV4, "outcome": ReflectingOutcome, "prediction_accuracy": z.enum(["correct","incorrect","partial"]), "post_mortem": z.string().min(1), "signal_retrospective": z.array(SignalReview), "adjustment": z.string().min(1) }));
